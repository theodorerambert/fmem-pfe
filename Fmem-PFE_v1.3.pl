# Theodore Rambert, Gabe Kahen, Timothy Lam
# Fmem-PFE
# Fmem Perl Front End
# Version 1.3
...
# Changelog
# 1.3	Added "Image Complete" Dialog
...
sub mem 
{
	...
	else
	{
		...
		if($flags[0] eq 1 && $flags[1] eq 1) #conv=noerror & sync chosen
		{
			...
			my $output = `dd if=/dev/fmem of=$curr_dir bs=1M count=$memsize conv=noerror,sync`;
		}
		elsif($flags[0] eq 1 && $flags[1] ne 1) #conv=noerror chosen
		{
			...
			my $output = `dd if=/dev/fmem of=$curr_dir bs=1M count=$memsize conv=noerror`;
		}
		elsif($flags[0] ne 1 && $flags[1] eq 1) #sync chosen
		{
			warn "[Info] Running: dd if=/dev/fmem of=$curr_dir bs=1M count=$memsize sync\n";
			my $output = `dd if=/dev/fmem of=$curr_dir bs=1M count=$memsize sync`;
		}
		else
		{
